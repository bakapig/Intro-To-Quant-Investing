...
On the other hand, regime‑switching models have gained prominence as another method to account for structural breaks in financial markets. Regime switching models that include Hidden Markov Models (HMM) are frequently used when it comes to modeling regimes of time series, including high and low volatility. According to the study conducted by Bae et al. \cite{bae2014} and Mulvey \& Liu \cite{mulvey2016}, stock market could be categorized into two states; normal state and crash state. In normal state, market is calm and rising. This state is also termed as “Bull Market”. On the other hand, in crash state, market is panic and falls down. It is referred to as “Bear Market”. HMMs are particularly attractive because they allow for probabilistic inference regarding the underlying regime and provide a systematic framework for detecting shifts in market conditions.
...
Several methodologies have been developed to estimate the Hurst exponent \cite{beran1994}, such as Rescaled Range (R/S) analysis, Detrended Fluctuation Analysis (DFA) \cite{peng1994}, and wavelet‑based methods. Kantelhardt et al. \cite{kantelhardt2002} subsequently extended this method to analyze multifractal time series (MFDFA), and Gu \& Zhou \cite{gu2006} extended it to multidimensional series. MFDFA can be compared to alternative methodologies, and applied across a broad spectrum of disciplines, including financial markets \cite{oswiecimka2005,yuan2009,thompson2016,xiong2017,teng2018}, and currency exchange rate analysis \cite{li2020,shang2020,li2021}. Although these methodologies differ in application, they all aim at assessing the scaling characteristics of time-series data across various time scales. However, there have been findings indicating that the estimation of the Hurst exponent is susceptible to factors such as the size of the sample, estimation horizon, and structural change within the data.
...
The strategy implements the following rule set based on defined thresholds, where $H_{\text{upper}} = 0.6$ (for strong persistence) and $H_{\text{lower}} = 0.4$ (for strong anti-persistence):
...
Finally, we analyze how strategy factors behave across these detected states. As seen in Figure \ref{fig:factor_regime}, momentum efficacy is heavily dependent on the underlying regime, with the highest alpha generated during persistent "Trending" states ($H > 0.6$).
...
To test the impact of strict capital preservation heuristics, an optional risk-gating module based on Dr. Alexander Elder’s trading frameworks \cite{elder2014} was integrated into the execution logic:
\begin{enumerate}
...
\begin{figure}[H]
    ...
    \caption{Cumulative Return Performance vs Starting Year across Strategies}
...
\section{Conclusion}
This paper presents a comprehensive theoretical and empirical framework that successfully bridges macroeconomic regime modeling with microstructural price behavior. By integrating a Gaussian Hidden Markov Model (HMM) to detect broad market regimes with the Hurst exponent to measure local trend persistence, the proposed architecture effectively filters out the noise and whipsaw signals that typically degrade traditional, naive momentum strategies. This cooperative strategy shows that signal robustness can be greatly increased by matching tactical momentum indicators with probabilistically predicted market settings.

However, a central finding of our backtesting analysis is that regime detection alone is insufficient to generate consistent outperformance. Empirical results show that the unscaled signal-driven weighting scheme (Standard HMM) suffered from severe drawdowns and yielded a negative Sharpe ratio. In contrast, pairing the regime-switching signals with volatility-sensitive position sizing—specifically through an Inverse VaR (Risk Parity) framework—drastically improved the strategy's viability. By dynamically scaling capital allocation inversely to forecasted downside risk, the HMM + VaR Parity execution managed to achieve the highest final portfolio value, tightly controlled drawdowns, and a superior Sharpe ratio. This highlights that adaptive risk budgeting is a mandatory component for regime-switching models to survive and capitalize on structural market breaks.

Expanding this concept from a time-series approach to a more comprehensive cross-sectional application holds great promise for future research. Calculating the Hurst exponent across the entire equity universe could allow for the construction of a novel "Smart Momentum Factor"—a portfolio going long exclusively on assets that rank highest in relative cross-sectional momentum while simultaneously exhibiting strong long-memory persistence ($H > 0.6$). Such advancements represent a promising frontier for multifactor quantitative modeling and adaptive portfolio management.
