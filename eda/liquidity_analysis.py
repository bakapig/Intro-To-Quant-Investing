...
# Amihud = |r_i,t| / DV_i,t   (×1e6 for readability)
abs_ret = returns.abs()
# avoid division by zero
dv_safe = dv.replace(0, np.nan)
amihud = (abs_ret / dv_safe) * 1e6  # scale for readability

# Cross-sectional median Amihud per day
...
# Monthly rebalance: at each month-end, sort into turnover quintiles,
# compute next-month equal-weighted return for each quintile
monthly_ret = returns.resample("ME").apply(lambda x: (1 + x).prod() - 1)
monthly_turnover = turnover.resample("ME").mean()  # avg daily turnover in month
...
# (b) Bar chart of annualized quintile returns
ann_rets = {}
for q in range(1, 6):
...
# Aggregate dollar volume (smoothed)
total_dv = dv.sum(axis=1)
total_dv_smooth = total_dv.rolling(20, min_periods=10).mean()

# Mark drawdown regimes: >-5% = normal, -5% to -20% = correction, < -20% = bear
regime = pd.cut(
    drawdown,
...
# (a) Drawdown with colored regimes
colors_map = {
    "Normal (>−5%)": "#a1d99b",
    "Correction (−20%→−5%)": "#fdae6b",
    "Bear (<−20%)": "#e34a33",
}
axes[0].fill_between(drawdown.index, drawdown.values, 0, color="lightcoral", alpha=0.4)
...
# (b) Dollar volume coloured by regime
...
# ── Summary table ────────────────────────────────────────────────────────────
# Avg turnover and Amihud by drawdown regime
summary_rows = []
...
summary.to_csv(os.path.join(OUTPUT_DIR, "20_liquidity_regime_summary.csv"), index=False)
