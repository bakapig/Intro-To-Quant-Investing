...
# Realized vol (annualized): sqrt(252 * rolling mean of r^2)
WINDOW = 20  # 1-month rolling

realized_var = (mkt_ret**2).rolling(WINDOW, min_periods=15).mean()
realized_vol = np.sqrt(realized_var * 252) * 100  # annualized %

# Parkinson volatility: uses high-low range
# For the market index we compute the cross-sectional weighted Parkinson per day
# Parkinson per stock: 1/(4*ln2) * (ln(H/L))^2
log_hl = np.log(high / low)
parkinson_var_stock = log_hl**2 / (4 * np.log(2))

# Market-level Parkinson: weighted average across stocks
...
parkinson_vol = np.sqrt(park_mkt_rolling * 252) * 100  # annualized %
...
for label, vs in vol_series.items():
    axes[0].plot(
        vs.index, vs.values, color=colors[label], linewidth=0.8, label=label, alpha=0.85
    )
axes[0].set_ylabel("Annualized vol (%)")
axes[0].set_title("Volatility Term Structure: 5d vs 20d vs 60d Realized Vol")
axes[0].legend(fontsize=9)

# (b) Term structure spread: 5d − 60d  (positive = inverted / stress)
...
# Correlation between r_t and vol_{t+1..t+20}
# Use forward 20-day realized vol
fwd_vol = (
    np.sqrt((mkt_ret**2).shift(-20).rolling(20, min_periods=15).mean() * 252) * 100
)
# Alternative: simply shift realized vol forward
fwd_vol_simple = realized_vol.shift(-WINDOW)

# Rolling 252-day correlation between return and future vol
...
# (a) Scatter (sampled weekly for clarity)
...
summary_df.to_csv(os.path.join(OUTPUT_DIR, "24_volatility_summary.csv"), index=False)
